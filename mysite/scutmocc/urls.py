...
urlpatterns = [
    ...
    url(r'^bbs/(?P<board_type>(?:activity|question|topic))/(?P<school_type>)/(?P<sort_type>(?:))$', views.bbs_board, name='bbs_board'),
    url(r'^bbs/(?P<board_name>(?:activity|question|topic))/(?P<theme_id>\d+)$', views.bbs_theme,
        name='bbs_theme')
]
