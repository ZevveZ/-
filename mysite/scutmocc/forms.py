...
from scutmocc.models import SubmitLes,LabelField
...
class SublesForm(forms.Form):
    name = forms.CharField(max_length=60)
    label = forms.ModelChoiceField(queryset=LabelField.objects.all(), to_field_name="Label_Name",
                                   empty_label="(Nothing)")
    intro = forms.CharField(max_length=120,
                            widget=Textarea(attrs={'cols': 80, 'rows': 20}))
    time = forms.IntegerField()
    price = forms.IntegerField()
    merge = forms.IntegerField()
    another = forms.CharField(max_length=200, widget=Textarea(attrs={'cols': 80, 'rows': 20}))

    def clean(self):
        cleaned_data = super(SublesForm, self).clean()
        pnum = cleaned_data.get('merge')
        if pnum is not None:
            if pnum > 10:
